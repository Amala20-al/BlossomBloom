...
@WebServlet("/editItemPriceAction")
public class EditItemPriceAction extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int item_price = Integer.parseInt(request.getParameter("item_price")); // Parse as integer if item_code is numeric

        try {
            // Load the JDBC driver
            Class.forName(driver);
            try (Connection conn = DriverManager.getConnection(url, user, pass);
                 PreparedStatement pst = conn.prepareStatement("UPDATE itemprice SET item_name=?, item_price=? WHERE item_id=?")) {

                // Prepare the SQL statement to update item details
                pst.setString(1, item_name);
                pst.setInt(2, item_price);
                ...
                if (updatedRows > 0) {
                    ...
                    response.sendRedirect("itempricedetails"); // Redirect to item management page after updating
                } else {
